...
Agent authorization is checked against the final scheduled installment plus a five-minute execution grace period. In **Local API agent**, review the expiry warning and choose **Authorize through final purchase** to request a wallet signature. An active agent can renew its permission; the app only extends its local expiry after verifying the exchange authorization. Expired or revoked agents are not silently reused. Strategies longer than the supported 179-day authorization window must be shortened before activation.
