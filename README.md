...
Agent authorization is checked against the final scheduled installment plus a five-minute execution grace period. In **Local API agent**, review the expiry warning and choose **Prepare replacement agent**, then **Authorize new agent in wallet** to request a wallet signature. Every replacement uses a fresh address stored separately. The app switches its active signer only after verifying the replacement address and its full requested lifetime on the exchange. Existing permissions are not automatically revoked; unused old agents can be removed in Hyperliquid API settings. Expired or revoked agents are not silently reused. Strategies longer than the supported 179-day authorization window must be shortened before activation.
